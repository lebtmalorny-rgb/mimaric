...
Standalone `mistral` имеет три дополнительные предметные команды относительно OSC 5.4.0: `action-validate`, `execution-get-sub-executions`, `task-get-sub-executions`. Источники, полная матрица и реестр: [mistral-heat.md](#mistral), [71+49 OSC entries](data/mistral-heat-osc-inventory.tsv), [standalone Mistral](data/mistral-standalone-inventory.tsv).
...
Подробная матрица, восемь команд evacuation guard, три команды Watcher guard и проверенные базы: [local-customizations.md](#custom). Происхождение локальных источников: [local-source-manifest.json](data/local-source-manifest.json).
...
Точные HEAD, состояние рабочих деревьев, SHA256 и комментарии архивов сохранены в [local-source-manifest.json](data/local-source-manifest.json). Наличие файлов в рабочем каталоге и история публикации не доказывают их установку в облаке. Текущий `main` имеет пользовательские изменения; они не редактировались.
...
В [power_ops.yaml](evidence/mistral-0809/etc/mistral/power_ops.yaml) определены четыре workflow: `host_power_status` (строка 6), `planned_power_off` (21), `planned_reboot` (41), `power_on_and_return` (60). В [setup.cfg](evidence/mistral-0809/setup.cfg), строки 60–65, зарегистрированы пять серверных actions:
...
1. [planned.py](evidence/mistral-0809/mistral/actions/powerops/planned.py), строка 38: `type(self.allow_hard_off) is not bool` отвергает строку `"false"`/`"true"`.
2. [clients.py](evidence/mistral-0809/mistral/actions/powerops/clients.py), строка 815: manifest остановленных ВМ должен быть списком, со строковыми идентификаторами и без повторов.
3. [return_host.py](evidence/mistral-0809/mistral/actions/powerops/return_host.py), строка 122: `stale_domains_checked is not True` блокирует возврат. Workbook берёт это значение из `env()`, строка 84.
...
По коду [automation_guard.py](evidence/watcher/watcher/common/automation_guard.py), строки 46–62, новые `ONESHOT`/`EVENT` планы обходят этот запрет; защищён автоматический путь `CONTINUOUS`. Уже допущенное действие может обратиться к Nova после hold. Это не drain, не отмена миграций и не общая блокировка всех ручных Mistral/Nova операций. Полная граница — [WATCHER-AUTOMATION-HOLD.md](https://github.com/lebtmalorny-rgb/mimaric/blob/c866d56fbf382afa4612c07039c34fbb5032aef5/docs/WATCHER-AUTOMATION-HOLD.md).
...
Источники: [MASAKARI-PER-TARGET-EVACUATION.md](https://github.com/lebtmalorny-rgb/mimaric/blob/71b0123f390bfeaab4dc4ca2dffaa6fab32a6cdf/docs/MASAKARI-PER-TARGET-EVACUATION.md), [manifest](https://github.com/lebtmalorny-rgb/mimaric/blob/71b0123f390bfeaab4dc4ca2dffaa6fab32a6cdf/hotfixes/masakari-per-target-evacuation/manifest.json), [интеграция Masakari](evidence/masakari/masakari/engine/drivers/taskflow/host_failure.py), строки 486–507.
...
`kolla-ansible enroll-ironic` зарегистрирован в [setup.cfg](evidence/kolla/setup.cfg), строка 65. В [enroll.yml](evidence/kolla/ansible/roles/ironic_enroll/tasks/enroll.yml), строки 161–176, заданы enroll, `network_interface=noop`, `storage_interface=noop`, `inspect_interface=no-inspect`, `bios_interface=no-bios`, `raid_interface=no-raid`. Завершающая цель — manageable.
...
Роль Horizon в Kolla передаёт пять отдельных флагов `ENABLE_HEAT`, `ENABLE_IRONIC`, `ENABLE_MASAKARI`, `ENABLE_MISTRAL`, `ENABLE_WATCHER` в [defaults/main.yml](evidence/kolla/ansible/roles/horizon/defaults/main.yml), строки 13–24. Это wiring установки/включения штатных плагинов; оно не доказывает фактическую доступность панели для пользователя. Нужны пакет в образе, enabled-файлы, endpoint, policy и соответствующие роли.
...
- [Объединённый первичный реестр](data/command-inventory.tsv).
- [Покомандная нормализованная матрица](data/full-cli-horizon-matrix.tsv).
- [Зафиксированные upstream-источники](data/upstream-source-manifest.json).
- [Архивы, SHA256 и HEAD собственных доработок](data/local-source-manifest.json).
- [Результат проверки полноты документа](data/verification.json).
