...
For enabled mode, preflight all VMoves for the notification before filtering PENDING: any existing FAILED VMove refuses dispatch and reserved-host activation. This prevents the existing reserved-host `ParameterizedForEach` retry from silently continuing the same failed recovery. Leave pending records intact for operator inspection; preserve the disabled retry behavior and do not add a VMove schema or redesign TaskFlow retries.
