...
Usage: python verify_ansible_preflight.py KOLLA_ANSIBLE_ROOT
...
def main():
    ...
    task_dir = root / 'ansible/roles/masakari/tasks'
    ...
    guard = copy.deepcopy(next(task for task in deploy if task.get('name') ==
                               'Verify selected Masakari hosts completed deploy'))
    ...
    # A local marker proves no later step can proceed after a failed check.
    # There is no catalogue population in the actual Masakari role.
    populate = {'name': 'Record successful gate', 'run_once': True}
    assert check['run_once'] is True and check['any_errors_fatal'] is True
    assert check['delegate_to'] == '{{ item }}'
    assert 'masakari.cmd.powerops_check' in check['ansible.builtin.command']
    assert '--config-file /etc/masakari/masakari.conf' in check['ansible.builtin.command']
    output = Path(tempfile.mkdtemp(prefix='masakari-0509-ansible-probe-', dir='/tmp'))
    ...
    scenarios = [
        ...
        ('limit-one-still-checks-three', 1, HOSTS[1], '', False),
    ...
    for name, serial, limit, fail_host, deploy_fail in scenarios:
        ...
        case_check['ansible.builtin.command'] = marker(
            events_path, 'CHECK', '{{ item }}', 'masakari_engine', fail_host)
        case_populate = copy.deepcopy(populate)
        case_populate['become'] = False
        case_populate['ansible.builtin.command'] = marker(events_path, 'ACCEPT',
                                                        '{{ inventory_hostname }}')
        ...
        accepted = [event for event in events if event['kind'] == 'ACCEPT']
        ...
        assertions = {
            ...
            'acceptance_count': len(accepted) == (1 if should_succeed else 0),
        }
        if not deploy_fail:
            assertions['all_three_replicas'] = {(e['host'], e['container']) for e in checked} == {
                (host, container) for host in HOSTS for container in CONTAINERS}
            assertions['exactly_three_checks'] = len(checked) == 3
        ...
        if should_succeed:
            assertions['checks_before_acceptance'] = bool(events) and all(
                e['kind'] == 'CHECK' for e in events[-4:-1]) and events[-1]['kind'] == 'ACCEPT'
        if deploy_fail:
            assertions['no_check_after_partial_deploy'] = not checked
            if serial == 0:
                assertions['guard_failed'] = 'Some selected Masakari hosts failed or became unreachable.' in log
        result = {'scenario': name, 'result': 'ok' if all(assertions.values()) else 'failed',
                  'rc': completed.returncode, 'checks': len(checked),
                  'acceptances': len(accepted), 'assertions': assertions}
