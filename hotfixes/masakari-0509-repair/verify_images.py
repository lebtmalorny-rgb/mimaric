#!/usr/bin/env python3
"""Check an already-built Masakari engine image offline; never start services.
...
root = Path(masakari.__file__).resolve().parent
if not expected or 'masakari/engine/drivers/taskflow/base.py' not in expected:
    raise RuntimeError('Missing expected runtime manifest')
for name, digest in expected.items():
    if not name.startswith('masakari/') or '..' in Path(name).parts:
        raise RuntimeError('Invalid runtime manifest path')
    path = root / name.removeprefix('masakari/')
...
subprocess.run([sys.executable, '-m', 'masakari.cmd.powerops_check'], check=True)
'''
...
def main():
    ...
    for role, reference in [('engine', args.engine_image)]:
        ...
        container_name = 'masakari-preflight-' + uuid.uuid4().hex
        try:
            subprocess.run([
    ...
    print('PASS: local Masakari image contract verified; live cloud untested')
    return 0
