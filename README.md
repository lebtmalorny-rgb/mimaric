...
- Активный PowerOps baseline 0809: интеграции Kolla/Ironic, Mistral и Masakari,
  плюс [Masakari post-fence Nova-down hotfix](https://github.com/lebtmalorny-rgb/mimaric/blob/main/hotfixes/masakari-post-fence-nova-down/README.md).
...
python -m pip install -r requirements.txt 'ansible-core==2.18.2'

python --version
ansible-playbook --version
python stand_test.py --help
```
...
Полный локальный набор: **75 тестов инструмента**.
...
cp examples/planned.json "$POWEROPS_CONFIG_DIR/planned.json"
cp examples/emergency.json "$POWEROPS_CONFIG_DIR/emergency.json"
...
```bash
python stand_test.py preflight \
  --task "$POWEROPS_CONFIG_DIR/emergency.json" --run-id ha-emergency-001 \
  --state-dir "$POWEROPS_STATE_DIR" --interface eno2 --duration 300
```
...
```bash
python stand_test.py report --run-id ha-emergency-001 --state-dir "$POWEROPS_STATE_DIR"
python stand_test.py report --run-id ha-planned-001 --state-dir "$POWEROPS_STATE_DIR"
```
...
## 9. Прочитать журнал или восстановить только сетевой интерфейс

Отдельный `link_fault.py` использует OpenSSH, SSH agent/config и `sudo -n`,
не читает inventory и не поддерживает интерактивный ввод пароля. Для входа
под root передайте `--no-sudo`. Чтобы работать с fault полного runner,
возьмите `operations.fault.nonce` из его JSON-отчёта и укажите те же run-id,
interface и duration. Имя в `--host` здесь — SSH-адрес, при необходимости с
пользователем, а не Nova host.

Через доступный SSH-путь можно прочитать состояние:

```bash
python link_fault.py status \
  --host operator@compute-01 --interface eno2 --run-id ha-emergency-001 \
  --duration 300 --intent-nonce REPLACE_WITH_NONCE_FROM_REPORT \
  --state-dir "$POWEROPS_STATE_DIR/link-fault"
```

Для досрочного восстановления интерфейса замените `status` на `restore`.
Это действие восстанавливает только исходный административный UP, если
идентичность интерфейса и boot не изменились. Оно не включает питание и не
снимает maintenance. Повторный `restore` не перезаписывает более поздние
изменения сети. Потерянный SSH-ответ означает неизвестный исход, а не отмену.

Сам worker регистрирует systemd `ExecStopPost` до DOWN, сохраняет исходные
boot ID, имя, ifindex и MAC, подтверждает DOWN и отсчитывает duration через
`CLOCK_BOOTTIME`. В конце он пытается вернуть UP; при аварийном завершении
systemd выполняет до трёх попыток восстановления. Выключение питания прерывает
оба механизма. Полный runner после загрузки сверяет fault-журнал/nonce
и закрывает журнал прежнего boot без изменения новой сети.
